...
class AxialPositionalEmbedding(nn.Module):
    ...
    def __init__(self, channels: int, spatial_size: Sequence[int]) -> None:
        ...
        self.pe = nn.ParameterList([nn.Parameter(torch.empty(1, channels, *[size if j == dim else 1 for j in range(len(spatial_size))])) for dim, size in enumerate(spatial_size)])
        for p in self.pe:
