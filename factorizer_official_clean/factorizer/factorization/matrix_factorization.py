...
class RandomInit(Initializer):
    ...
    def __init__(self, rank: int, size: tuple[int, int], method: str | tuple[str, str]='uniform') -> None:
        super().__init__()
...
class BCDSolver(nn.Module):
    ...
    def forward(self, x: Tensor, factor_matrices: tuple[Tensor, Tensor]) -> tuple[Tensor, Tensor]:
        u, v = factor_matrices
...
class LeastSquares(BCDSolver):
    ...
    def __init__(self, factor: Sequence[int]=(0, 1), eps: float=1e-16, project: Optional[PartialModuleType]=None, **kwargs):
...
class ProjectedGradient(BCDSolver):
    ...
    def __init__(self, factor: Sequence[int]=(0, 1), project: Optional[PartialModuleType]=None, eps: float=1e-16, **kwargs):
        super().__init__(factor=factor)
        self.eps = eps
        project = nn.Identity if project is None else project
        project = partialize(project)
...
class MultiplicativeUpdate(BCDSolver):
    ...
    def __init__(self, factor: Sequence[int]=(0, 1), eps: float=1e-16, **kwargs) -> None:
        super().__init__(factor=factor)
        self.eps = eps
...
class FastMultiplicativeUpdate(BCDSolver):
    ...
    def __init__(self, factor: Sequence[int]=(0, 1), eps: float=1e-16, **kwargs):
        super().__init__(factor=factor)
        self.eps = eps
...
class WeightedMultiplicativeUpdate(BCDSolver):
    ...
    def forward(self, x: Tensor, factor_matrices: tuple[Tensor, Tensor], w: Optional[Tensor]=None) -> tuple[Tensor, Tensor]:
        u, v = factor_matrices
...
class SVD(nn.Module):
    ...
    def __init__(self, size: tuple[int, int], rank: Optional[int]=None, compression: float=10, no_grad: bool=False, verbose: bool=False) -> None:
        super().__init__()
        self.size = M, N = size
        self.no_grad = no_grad
        assert (rank, compression) != (None, None), "'rank' or 'compression' must be specified."
        df_input = M * N
        df_lowrank = M + N
        if rank is None:
            self.rank = R = rank = max(math.ceil(df_input / (compression * df_lowrank)), 1)
        else:
            self.rank = R = rank
...
class MatrixFactorization(nn.Module):
    ...
    def __init__(self, size: Sequence[int], rank: Optional[int]=None, compression: float=10, init: str | PartialModuleType='normal', solver: str | PartialModuleType | Sequence[str | PartialModuleType]='cd', num_iters: int=5, num_grad_steps: Optional[int]=None, verbose: bool=False, **kwargs) -> None:
        super().__init__()
        self.size = M, N = size
    ...
    def loss(self, x: Tensor, u: Tensor, v: Tensor, w: Optional[Tensor]=None) -> Tensor:
...
class NMF(MatrixFactorization):
    ...
    def __init__(self, size: tuple[int, int], rank: Optional[int]=None, compression: float=10, num_iters: int=5, num_grad_steps: Optional[int]=None, init: str | PartialModuleType='uniform', solver: str | PartialModuleType | Sequence[str | PartialModuleType]='hals', verbose: bool=False, **kwargs):
        super().__init__(size, rank=rank, compression=compression, num_iters=num_iters, num_grad_steps=num_grad_steps, init=init, solver=solver, verbose=verbose, **kwargs)
INIT_DISPATCH_MAP = {'uniform': (RandomInit, {'method': 'uniform'}), 'normal': (RandomInit, {'method': 'normal'}), 'normal-uniform': (RandomInit, {'method': ('normal', 'uniform')}), 'uniform-normal': (RandomInit, {'method': ('uniform', 'normal')}), 'svd': SVDInit, 'nndsvd': NNDSVDInit}
SOLVER_DISPATCH_MAP = {'mu': MultiplicativeUpdate, 'mu-0': (MultiplicativeUpdate, {'factor': 0}), 'mu-1': (MultiplicativeUpdate, {'factor': 1}), 'fmu': FastMultiplicativeUpdate, 'fmu-0': (FastMultiplicativeUpdate, {'factor': 0}), 'fmu-1': (FastMultiplicativeUpdate, {'factor': 1}), 'wmu': WeightedMultiplicativeUpdate, 'wmu-0': (MultiplicativeUpdate, {'factor': 0}), 'wmu-1': (MultiplicativeUpdate, {'factor': 1}), 'smu': SemiMultiplicativeUpdate, 'smu-0': (SemiMultiplicativeUpdate, {'factor': 0}), 'smu-1': (SemiMultiplicativeUpdate, {'factor': 1}), 'cd': CoordinateDescent, 'cd-0': (CoordinateDescent, {'factor': 0}), 'cd-1': (CoordinateDescent, {'factor': 1}), 'nncd': (CoordinateDescent, {'project': nn.ReLU}), 'nncd-0': (CoordinateDescent, {'factor': 0, 'project': nn.ReLU}), 'nncd-1': (CoordinateDescent, {'factor': 1, 'project': nn.ReLU}), 'hals': (CoordinateDescent, {'project': nn.ReLU}), 'hals-0': (CoordinateDescent, {'factor': 0, 'project': nn.ReLU}), 'hals-1': (CoordinateDescent, {'factor': 1, 'project': nn.ReLU}), 'ls': LeastSquares, 'ls-0': (LeastSquares, {'factor': 0}), 'ls-1': (LeastSquares, {'factor': 1}), 'nnls': (LeastSquares, {'project': nn.ReLU}), 'nnls-0': (LeastSquares, {'factor': 0, 'project': nn.ReLU}), 'nnls-1': (LeastSquares, {'factor': 1, 'project': nn.ReLU})}
...
def _parse_solver(obj: str | PartialModuleType | Sequence[str | PartialModuleType]) -> PartialModuleType:
