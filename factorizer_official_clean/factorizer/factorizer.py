...
class FactMixer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, spatial_size, reshape=(Matricize, {'num_heads': 1, 'grid_size': 1}), act=nn.ReLU, factorize=NMF, dropout=0.0, **kwargs):
        super().__init__()
...
class FactorizerBlock(nn.Module):
    ...
    def __init__(self, channels, spatial_size, norm=LayerNorm, dropout=0.0, mlp_ratio=2, **kwargs):
        super().__init__()
...
class FactorizerStage(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, spatial_size, depth=1, adapter=(Linear, {'bias': False}), pos_embed=nn.Identity, dropout=0.0, **subblocks):
        super().__init__()
        ...
        for _ in range(depth):
            self.blocks.append(FactorizerBlock(out_channels, spatial_size, **subblocks))
...
class Factorizer(UNet):
    ...
    def __init__(self, in_channels, out_channels, spatial_size, encoder_depth=(1, 1, 1, 1, 1), encoder_width=(32, 64, 128, 256, 512), strides=(1, 2, 2, 2, 2), decoder_depth=(1, 1, 1, 1), stem=None, downsample=None, upsample=None, head=None, pos_embed=PositionalEmbedding, num_deep_supr=False, **kwargs):
        if stem is None:
            stem = (getattr(nn, f'Conv{len(spatial_size)}d'), {'kernel_size': 3, 'padding': 1, 'bias': False})
        ...
        super().__init__(in_channels, out_channels, spatial_dims=len(spatial_size), spatial_size=spatial_size, encoder_depth=encoder_depth, encoder_width=encoder_width, strides=strides, decoder_depth=decoder_depth, stem=stem, downsample=downsample, block=block, upsample=upsample, head=head, num_deep_supr=num_deep_supr)